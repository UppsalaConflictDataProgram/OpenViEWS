...
class Crosslevel():
    ...
    def _load_run(self, run):
        """ Load the run-specific attributes"""
    ...
    def _load_local_settings(self, local_settings):
        """ Load node specific local settings, such as connectstring"""
    ...
    def load_local_settings_file(self, path_local_settings):
        with open(path_local_settings, 'r') as f:
            local_settings = json.load(f)
            self._load_local_settings(local_settings)

    def fetch_data(self):
        pass
        #df_h = dbutils.db_to_df(self.connectstring, schema=)
